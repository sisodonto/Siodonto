...
abstract class Dbrecord_Core{
    ...
    */
    ...
    public function find_by_column($column_filter, $column_compare, $value_compare){
    ...
    public function find_by_column_distinct($column_filter, $column_compare, $value_compare){
        $table = strtolower(substr(get_class($this), 0, -6));
        if (is_string($value_compare)) {
            $where = "{$column_compare} = '{$value_compare}'";
        } else {
